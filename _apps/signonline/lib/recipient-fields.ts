...
export function recipientFields(original: any[], proposed: any[], email: string, pages: number) {
 ...
 const editable=(f:any)=>f.email===email&&!f.value&&!f.preparedText&&!f.signedAt&&!f.signedBy;
 ...
 if(f.email!==email||f.preparedText||f.value||f.signedAt||f.signedBy)throw Error('You can edit only your own unfinished fields.');
